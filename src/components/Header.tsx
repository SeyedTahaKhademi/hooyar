...
export const Header: React.FC<HeaderProps> = ({
  ...
  const renderSubMenu = (menu: MenuItemDef, menuIdx: number) => {
    ...
    return (
      <div
        className="absolute top-full right-0 mt-0.5 w-64 py-1 rounded-lg shadow-2xl border border-slate-700/60 bg-[#111827] backdrop-blur-xl z-[9999] overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150"
        style={{ minWidth: '240px' }}
        onClick={(e) => e.stopPropagation()}
      >
        {menu.subItems.map((item, idx) => (
          item.divider ? (
            <div key={`div-${idx}`} className="my-1 border-t border-slate-700/50" />
          ) : (
            <button
              key={idx}
              onClick={() => {
              ...
              className={`w-full flex items-center justify-between gap-3 px-3 py-1.5 text-right hover:bg-sky-600/90 hover:text-white transition-colors group ${
                item.danger ? 'hover:bg-rose-600/90 text-rose-300' : 'text-slate-200'
              }`}
            >
              <div className="flex items-center gap-2">
                {item.icon && <item.icon className="w-3.5 h-3.5 text-slate-400 group-hover:text-white/90 shrink-0" />}
                <span className="text-[11.5px] font-medium">{item.label}</span>
              </div>
              {item.shortcut && (
                <span className="text-[10px] text-slate-500 group-hover:text-white/70 font-mono dir-ltr">
                  {item.shortcut}
                </span>
              )}
            </button>
  ...
  return (
    <header className="h-10 border-b border-slate-800/90 bg-[#0d1117] flex items-center justify-between select-none z-50 relative custom-titlebar">
      ...
      <div className="flex items-center h-full flex-1 min-w-0 pr-2" ref={menuRef}>
        {/* App Logo */}
        <div
          className="flex items-center gap-2 px-3 h-full hover:bg-slate-800/50 transition-colors cursor-default no-drag"
          ...
          {menuDefinitions.map((menu, idx) => (
            <div key={idx} className="relative h-full">
              <button
                ...
                className={`px-3 h-full text-[11px] transition-colors focus:outline-none ${
